...
# sys.path.insert(0, f'{root_dir}/src/stretch_controller')
...
class Groundings:
    def __init__(self, files_json, objects_data: dict, locations_data: dict, user_inputs_data: dict, controller = None, create_symbols_only: bool = CREATE_SYMBOLS_ONLY):
        ...
        elif isinstance(files_json, dict):
            self.files = files_json
        else:
            raise ValueError(f'files_json {files_json} must be a string or a dictionary, but got {type(files_json)}')
        self.inputs = self.create_inputs()
        self.create_symbols_only = create_symbols_only
        if create_symbols_only: return None
    ...
    def create_inputs(self) -> list:
        ...
        if True:
            print("===== Objects =====")
            print(list(self.objects_data.keys()))
            print("===== Regions =====")
            print(list(self.locations_data.keys()))
        ...
        inputs_names = list(self.inputs_data.keys())

        dump_json(self.files['inputs_name'], {"inputs": inputs_names})
        dump_json(self.files['inputs_data'], self.inputs_data)
...
def test_pickdrop_block_thread(main_controller, groundings: Groundings):
    rospy.loginfo('Reaching default pose')
    main_controller.arm_controller.reach_default()
    poses = main_controller.get_poses()
