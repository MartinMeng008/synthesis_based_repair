    ...
    def run_symbolic_repair(self) -> dict:
        ...
        while not is_realizable and cnt <= self.max_iter:
            ...
            if is_realizable:
                ...
                return skills
            self.compiler.remove_skills()
            # Here we do not set manipulations or mobile only, because a repair may consist of both for laundry example
            ...
            if "max_repair_time" in self.opts.keys() and time_used > self.opts["max_repair_time"]:
                print("==== Time limit exceeds ====")
                print(f"Cannot finish after {time_used}")
                return {}
        return skills
...
def test_add_violation_and_repair_with_backup_skills(filename_structuredslugsplus, opts, files):
    skills_data = json_load_wrapper(files["skills"])
    symbols_data = json_load_wrapper(files["inputs"])
    objects_data = json_load_wrapper(files["objects"])
    controllable_symbols = find_controllable_symbols(symbols_data, objects_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-b', "--baseline", action='store_true', dest='baseline', required=False, help='baseline for LLM-repair', default=False)
