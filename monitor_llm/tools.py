...
def find_uncontrollable_objects(objects_data):
...
def load_list_keys_from_json(json_file: str) -> list:
    ...
    return load_list_keys_from_dict(inputs_data)

def load_list_keys_from_dict(inputs_data: dict) -> list:
    """Load inputs list from json file"""
...
def find_controllable_mobile_symbols(symbols_data, objects_data):
    ...
    if DEBUG:
        ...
        print("symbols data: ", symbols_data)
        print("==== exit due to debug ====")
        sys.exit(0)
    # raise Exception("Check objects_data")
    return [s for s, s_data in symbols_data.items() 
            if s_data and 
            (objects_data[s_data['object']]['category'] != "uncontrollable" and
...
def find_controllable_manipulation_symbols_by_not_object(obj: str, symbols_data: dict, objects_data: dict, controllable_manipulation_inputs: list = None) -> list:
    """Find controllable manipulation symbols whose object is not the given object"""
    if controllable_manipulation_inputs:
        return [s for s in controllable_manipulation_inputs if symbol_to_object(s) != obj]
...
def find_robot_locations(locations_data: dict) -> dict:
    """Return dicts of locations with is_robot_location == True"""
    return {loc: loc_data for loc, loc_data in locations_data.items() if loc_data['is_robot_location']}
...
def find_mobile_objects(objects_data: dict) -> dict:
    """Return dicts of objects with type == mobile"""
    return {obj: obj_data for obj, obj_data in objects_data.items() if obj_data['type'] == 'mobile'}
...
def filer_ee_from_inputs(inputs: list) -> list:
    """Filter out inputs that ends with ee"""
    return [inp for inp in inputs if not inp.endswith('ee')]
...
def run_slugs(filename_slugsin: str, plot = False, counterstrategy=False, is_realizable_only=False) -> dict:
    """Given a slugsin file, return the strategy produced by slugs"""
    print("Generate strategy")
    if is_realizable_only:
        result = subprocess.run(['slugs', '--cooperativeGR1Strategy',
                            filename_slugsin], capture_output=True, text=True)
        # result = subprocess.run(['slugs',
        #                     filename_slugsin], capture_output=True, text=True)
        if not "unrealizable" in result.stderr:
            if plot:
                plot_strategy(result.stdout)
        return result
    if counterstrategy:
    ...
    if not plot:
        ...
        if not "unrealizable" in result.stderr:
            if plot:
                plot_strategy(result.stdout)
        # sys.path.insert(0, '/home/qian/workspace/slugs/tools/')
        # pass
...
def generate_counterstrategy(filename_slugsin: str) -> dict:
    """Given a slugsin file, return the strategy produced by slugs"""
    print("Generate counterstrategy")
    result = subprocess.run(['slugs', '--counterStrategy',
                            filename_slugsin], capture_output=True, text=True)
    return result

def run_slugs_with_min_json_output(filename_slugsin: str, plot = False, counterstrategy=False) -> dict:
    """Given a slugsin file, return the strategy produced by slugs"""
    print("Generate strategy")
    if counterstrategy:
        result = subprocess.run(['slugs', '--counterStrategy',
        ...
        if plot:
            try:
                states, transitions = plot_strategy(result.stdout)
            except:
                print("Error in plotting strategy")
                return result, None, None
        return result, states, transitions
    else:
        result = subprocess.run(['slugs', '--explicitStrategy', '--jsonOutput', '--cooperativeGR1Strategy',
                            filename_slugsin], capture_output=True, text=True)
        # result = subprocess.run(['slugs', '--explicitStrategy','--jsonOutput',
        #                     filename_slugsin], capture_output=True, text=True)
        if plot:
            try:
                states, transitions = plot_strategy(result.stdout)
        ...
        if "unrealizable" in result.stderr:
            return result
        result.stdout = json.dumps(modify_json_for_readability(json_parser(result.stdout))) # indent=2
    return result

def modify_json_for_readability(strategy_json: dict) -> dict:
    """Modify the json for readability"""
    print(type(strategy_json))
    strategy_json.pop("version")
    strategy_json.pop("slugs")
    vars: list = strategy_json.pop("variables")
    # vars: list = strategy_json['variables']
    nodes: list = strategy_json['nodes']
    for node_idx, node_data in nodes.items():
        simplify_node_state_for_readability(node_data, vars)
    return strategy_json

def simplify_node_state_for_readability(node_data: dict, vars: list) -> None:
    """Simplify the node state for readability"""
    old_state = node_data['state']
    new_state = []
    for idx, val in enumerate(old_state):
        if val == 1:
            new_state.append(vars[idx])
...
def plot_strategy(strategy: str) -> None:
    ...
    for line in strategy:
        if line.startswith("State"):
            ...
            if not propositions is None:
                if propositions == thesePropositions:
    ...
    # Parsing done. Write
    output = "digraph {" + "\n"
    # output += "node [shape=tab] \""+",".join(propositions)+"\";" + "\n"

    for (state,(rank,values)) in states.items():
        output += "\""+state+"\" [shape=box,label=\""+",".join(prop_val_to_state(propositions, values))+"/"+rank+"\"];" + "\n"
        states[state] = (rank,prop_val_to_state(propositions, values))
    for state in states:
        if state in transitions:
            for b in transitions[state]:
                output += "\""+state+"\" -> \""+str(b)+"\";" + "\n"   
        else:
            print("No transition for state "+state)
    output += "}"

    plot_graphz(output)
    return states, transitions

def plot_graphz(output: str) -> None:
    """Plot the graph"""
    s = Source(output, filename='build/strategy.gv', format='png')
    s.view()

def prop_val_to_state(propositions: list, values: list) -> list:
    """Convert proposition values to state"""
    return [propositions[i] for i in range(len(values)) if values[i] == "1"]

def check_slugsin_realizability(filename_slugsin: str, plot=False, counterstrategy=False, is_realizable_only=False) -> bool:
    """Given a slugsin file, check if the specification is realizable"""
    print("Checking realizability")
    result = run_slugs(filename_slugsin, plot=plot, counterstrategy=counterstrategy, is_realizable_only=is_realizable_only)
    if DEBUG:
        print(result.stdout)
    print(result.stderr)
    return not "unrealizable" in result.stderr

def check_slugsin_realizability_with_readable_strategy(filename_slugsin: str, plot: bool = False, counterstrategy: bool = False) -> bool:
    """Given a slugsin file, check if the specification is realizable"""
    print("==== Checking realizability ====")
    result = run_slugs_with_min_json_output(filename_slugsin, plot=plot, counterstrategy=counterstrategy)
    if counterstrategy:
        assert len(result) == 3, "The result should have 3 elements with realizability check with counterstrategy"
        result, states, transitions = result
    if DEBUG:
        print("== stdout ==")
        print(result.stdout)
        print("")
        ...
        print(result.stderr)
    if counterstrategy:
        return not "unrealizable" in result.stderr, result.stdout, states, transitions
    return not "unrealizable" in result.stderr, result.stdout
...
def read_txt_file(file: str) -> str:
    """Read a txt file"""
    try:
...
def list2dict_bool_true(ls: list) -> dict:
    dic = {}
    for name in ls:
        dic[name] = True
    return dic

def state2dict_bool(inputs: list, inp_state: list, true_only: bool = False) -> dict:
    if true_only:
        return list2dict_bool_true(inputs)
    dic = list2dict_bool(inputs)
    for inp in inp_state:
        dic[inp] = True
    return dic

def dict_bool2list_dic_only(dic: dict) -> list:
    ls = []
...
def dict_bool2list(dic: dict, care_vars: list = None) -> list:
    if care_vars is None:
        return dict_bool2list_dic_only(dic)
    ls = []
    for name in care_vars:
        if dic[name]:
            ls.append(name)
    return ls
...
def print_bdd(bdd: _bdd.BDD, expr):
...
def state2state_prime(state: list) -> list:
    """Convert state to state prime"""
...
def write_section(file_spec, section_name, section_spec):
    ...
    if isinstance(section_spec, list):
        for line in section_spec:
            fid.write(line)
            fid.write("\n")
    elif isinstance(section_spec, str):
        fid.write(section_spec)
    else:
        raise Exception("Section spec should be either list or string")
    fid.write("\n")
    fid.close()
...
def find_changing_input(pre: dict, post: dict) -> str:
    ...
    for sym, val in pre.items():
        if (val and sym not in post.keys()) or val != post[sym]:
            return sym
    for sym, val in post.items():
        if (val and sym not in pre.keys()) or val != pre[sym]:
            return sym
    # try: 
...
## ==== Helper function for Game structure ==== ##
def create_expr(arg_bdd: _bdd.BDD, exprs: list) -> _bdd.Function:
    t = arg_bdd.true
    for e in exprs:
        try:
...
def is_prime(var: str) -> bool:
    return var[-1] == "'"

def unprimed(var: str) -> str:
    return var[:-1]

def get_states_from_edge(edge: tuple) -> list:
    """Get the states from the edge"""
    u, v = [], []
    for var in edge:
        if is_prime(var):
            v.append(unprimed(var))
        else:
            u.append(var)
    return tuple(u), tuple(v)

def create_state_from_vars_and_values(vars: list, values: list) -> list:
    """Creates a set of true vars"""
    return [vars[i] for i in range(len(vars)) if values[i]]

def form(bdd: _bdd.BDD, S: list, S_prime: list) -> _bdd.Function:
    """form(S, S') := AND_(v in S') v & And_(v in S \ S_prime) !v"""
    S_prime_expr = create_expr(bdd, S_prime)
    not_vars = [f'!{v}' for v in S if v not in S_prime]
    S_expr = create_expr(bdd, not_vars)
    return S_prime_expr & S_expr

def produce_strategy_json(compiler) -> dict:
    """Produce strategy from structuredslugs file"""
    slugsin_file = "build/tmp.slugsin"
    compiler.generate_slugsin(slugsin_file)
    return produce_strategy_json_from_slugsin(slugsin_file)
    

def produce_strategy_json_from_slugsin(slugsin_file: str) -> dict:
    """Produce strategy from slugsin file"""
    result = run_slugs(slugsin_file)
    if "unrealizable" in result.stderr:
        raise Exception("Specification is unrealizable")
    return json_parser(result.stdout)
