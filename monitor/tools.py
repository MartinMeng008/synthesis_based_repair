...
def create_symbols_from_objects_and_locations(objects_data: dict, locations_data: dict) -> list:
    ...
    # robot_objects = objects_data['robot_objects']
    # uncontrollable_objects = objects_data['uncontrollable_objects']
    # objects_names = list(objects_data.keys())
    locations_names = list(locations_data.keys())
...
def create_symbol_dict(sym_name, obj, loc, index_cnt, location_data = None):
    ...
    if location_data:
        sym_dict['dims'] = location_data[loc]['dims']
        sym_dict['bounds'] = location_data[loc]['bounds']
...
def find_symbols_by_object(obj: str, symbols_data: dict) -> list:
    ...
    return [s for s, s_data in symbols_data.items() if s_data['object'] == obj]
...
def find_symbols_by_location(loc: str, symbols_data: dict, locations_data, objects_data) -> list:
    ...
    return [s for s, s_data in symbols_data.items() if s_data['location'] == loc and (objects_data[s_data['object']]['type'] == locations_data[loc]['type'] or (locations_data[loc]['type']  == 'manipulation' and objects_data[s_data['object']]['type'] == 'any'))]
...
def find_symbols_by_objects_type_category(type, symbols_data, objects_data):
    ...
    return [s for s, s_data in symbols_data.items() 
            if objects_data[s_data['object']]['category'] != "uncontrollable" and
...
def get_nonrobot_objects(objects_data: dict) -> list:
    ...
    return [obj for obj, obj_data in objects_data.items() if obj_data['category'] != 'robot']
...
def find_true_robot_mobile_symbols(X: dict, symbols_data: dict, objects_data: dict, locations_data: dict) -> list:
    ...
    for sym in true_symbols:
        if objects_data[symbols_data[sym]['object']]['type'] == 'mobile' and \
           objects_data[symbols_data[sym]['object']]['category'] == 'robot' and \
...
def find_true_robot_manipulation_symbols(X: dict, symbols_data: dict, objects_data: dict, locations_data: dict, object: str) -> list:
    ...
    for sym in true_symbols:
        if symbols_data[sym]['object'] == object or \
