...
from symbolic_repair_base import run_repair
...
class Repair:
    ...
    def run_symbolic_repair(self) -> dict:
        ...
        while not is_realizable and cnt <= self.max_iter:
            ...
            # 1. Give a new seed for each iteration
            np.random.seed(seed)
            seed += 1

            # 2. Check realizability
...
def rename_new_skills(compiler: Compiler, skills: dict) -> None:
    ...
    # compiler.remove_last_added_skills()
    rename_skills(compiler, skills)

def rename_skills(compiler: Compiler, skills: dict) -> None:
